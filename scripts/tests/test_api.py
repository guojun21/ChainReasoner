...
def test_brave_search():
    """Test brave-search MCP service"""
    print("\nTesting brave-search MCP service...")
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                if "error" not in response_data:
                    print(f"  ✓ Brave search passed with {response_data.get('count', 0)} results")
                    return True
                else:
                    print(f"  ✗ Brave search failed: {response_data['error']}")
                    return False
            else:
                print(f"  ✗ Brave search failed with status code: {response.status_code}")
                return False
...
if __name__ == "__main__":
    ...
    print(f"  - Brave Search: {'✓ PASS' if brave_ok else '✗ FAIL'}")
    print(f"  - SearXNG: {'✓ PASS' if searxng_ok else '✗ FAIL'}")
    
    all_passed = health_ok and mcp_list_ok and brave_ok and searxng_ok
